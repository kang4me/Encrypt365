...
import com.kang.service.Impl.RsaEncryptImpl;
...
public class RsaEncryptUI {
    ...
    private final RsaEncrypt rsaPublicKeyEncrypt = new RsaEncryptImpl();
    private RsaEntity rsaEntity;
    private final MontoyaApi api;

    public RsaEncryptUI(MontoyaApi api) {
        this.api = api;
        ini();
    ...
    private void encodeButton() {
        try {
            ...
            this.encodeText.setText("参数异常，请检查");
    ...
    private void decodeButton() {
        try {
            ...
            if (privateKeyRadio.isSelected()) {
                this.encodeText.setText(rsaPublicKeyEncrypt.decodeWithPrivateKey(rsaPublicKeyEncrypt.getPrivateKeyFromPEM(rsaEntity.getPublicKey()), rsaEntity));
            }
        } catch (Exception e) {
            this.encodeText.setText("参数异常，请检查");
...
/*        if (this.padding.getSelectedIndex() == 0) {//PKCS1_PADDING
    ...
    private void ini() {
        ...
        publicKey.setText("公钥：（以“-----BEGIN PUBLIC KEY-----”开头 “-----END PUBLIC KEY-----” 结尾）\n"
